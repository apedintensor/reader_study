...
@router.get("/case/{case_id}", response_model=List[schemas.AIOutputRead])
...
@router.post("/", response_model=schemas.AIOutputRead, status_code=201)
def create_ai_output(
    *, # Enforces keyword-only arguments
    db: Session = Depends(deps.get_db),
    ai_output_in: schemas.AIOutputCreate
):
